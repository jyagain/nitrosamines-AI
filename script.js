...
let activeSubTab = 'adi'; // 'adi' or 'predict'
...
function switchSubTab(subTab) {
    ...
    const btnAdi = document.getElementById('subTabBtnAdi');
    const btnPredict = document.getElementById('subTabBtnPredict');
    
    const contentAdi = document.getElementById('subTabContentAdi');
    ...
    if (subTab === 'adi') {
        ...
        contentAdi.classList.add('active');
        contentPredict.classList.remove('active');
    } else {
        btnAdi.classList.remove('active');
        btnPredict.classList.add('active');
        contentAdi.classList.add('active');
        contentPredict.classList.add('active');
...
function runPredictCPCA() {
    ...
    checkPredictSMILESMatch(smiles, (match) => {
        if (match) {
            ...
            warningContainer.innerHTML = `
                <div class="alert-title" style="color: #b45309; display: flex; align-items: center; gap: 0.5rem; font-weight: 700;">
                    <i class="fa-solid fa-triangle-exclamation"></i> 주의: 식약처 설정 기준 존재 물질
                </div>
                <div class="alert-body" style="font-size: 0.95rem; line-height: 1.5; margin-top: 0.5rem; color: #78350f;">
                    이 물질은 식약처에서 섭취허용량 기준을 이미 설정하여 발표한 물질(<strong>${match.name}</strong>, AI: <strong>${match.ai} ng/day</strong>)과 일치하거나 매우 유사합니다.<br>
                    따라서 CPCA 예측 결과보다 식약처가 공식 발표한 허용량 기준을 우선 준수해야 합니다.
                    <button class="btn btn-secondary btn-sm" style="margin-top: 0.75rem; display: flex; align-items: center; gap: 0.4rem; font-size: 0.82rem;" onclick="window.selectAndAnalyzeItemByNo(${match.no}); switchSubTab('adi');">
                        <i class="fa-solid fa-building-shield"></i> 식약처 설정 기준 데이터 보기
                    </button>
                </div>
            `;
...
function checkPredictSMILESMatch(smiles, callback) {
    // 1. Local Preset database direct matching
...
function printResult() {
    ...
    document.getElementById('printMode').textContent = (activeTab === 'auto') 
        ? (activeSubTab === 'adi' ? "식약처 기준 설정 니트로사민류 조회" : "신규 니트로사민류 CPCA 예측") 
        : "자가진단 수동 위자드";
